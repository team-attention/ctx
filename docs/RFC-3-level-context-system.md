# RFC: 3-Level Context System

> CTX 프로젝트 방향 전환 제안서
> 작성일: 2025-12-29
> 작성자: Claude (Opus 4.5)
> 최종 수정: 2026-01-02 (ctx-load Skill 추가 - 수동 로드용)

---

## 1. 배경 및 문제 정의

### 현재 상황 (As-Is)

CTX는 "팀 단위 개발 협업 플랫폼"으로 설계되었으나, 실제 사용 경험에서 다음 문제들이 발견됨:

#### 과잉 설계 (Over-Engineering)
```
src/templates/ai-commands/work/   # ❌ 제거 대상
├── init.md      # 이슈 초기화 (GitHub/Linear/Local 분기)
├── plan.md      # 구현 계획
├── extract.md   # 컨텍스트 추출
├── review.md    # 코드 리뷰
├── commit.md    # 커밋
├── submit.md    # PR 생성
└── done.md      # 완료

+ issue_store: local | github-issue | linear  # ❌ 제거
+ worktree 지원                                # ❌ 제거
+ session tracking hooks                       # 단순화
```

#### 사용 패턴과 설계 의도의 괴리

| 설계 의도 | 실제 사용 |
|-----------|-----------|
| 피처별 브랜치 + PR + 리뷰 | 한 브랜치에서 여러 피처 |
| 팀원간 컨텍스트 공유 | 혼자라서 공유할 대상 없음 |
| 이슈 기반 원자적 작업 | work 단위가 비대해짐 |
| 세션마다 컨텍스트 추출 | 까먹거나 필요성 못 느낌 |

#### 핵심 문제
- **"사용하게 만드는 힘"이 약함**
- 워크플로우가 무거워서 건너뛰게 됨
- 컨텍스트가 자동으로 활용되지 않음
- 저장/추출의 타이밍이 애매함

---

## 2. 제안: 3-Level Context System

### 방향 전환
- **개발 협업 도구 → 범용 컨텍스트 관리 플랫폼**
- **수동 로드 → 자동 주입**
- **복잡한 워크플로우 → 단순한 핵심 기능만**
- **분산 Registry → 하이브리드 Registry** (개인 + 팀 협업 모두 지원)
- **config.yaml 제거 → registry.yaml 단일화** (설정 최소화)

### Level 구조

```
┌─────────────────────────────────────────────────────────────┐
│                    3-Level Hierarchy                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Global (~/.ctx/)                           [필수, Root]    │
│  └─ 사용자 전역, 모든 프로젝트에서 참조                    │
│  └─ 개인 코딩 스타일, 범용 지식, 도구 사용법 등            │
│  └─ ctx init 으로 최초 1회 생성                            │
│  └─ registry.yaml만 존재 (config 없음!)                    │
│  └─ Git: ❌ (개인 머신 종속)                               │
│                                                             │
│       ↓ Load 시 함께 탐색                                   │
│                                                             │
│  Project ({project}/.ctx/)                                  │
│  └─ 프로젝트 단위 컨텍스트                                 │
│  └─ 아키텍처, API 패턴, 비즈니스 로직 등                   │
│  └─ ctx init . 으로 생성                                   │
│  └─ registry.yaml만 존재 (config 없음!)                    │
│  └─ Git: ✅ (팀 협업 지원)                                 │
│                                                             │
│       ↓ 상속/오버라이드                                     │
│                                                             │
│  Local (*.ctx.md, ctx.md)                                   │
│  └─ 특정 파일/폴더에 붙은 컨텍스트                         │
│  └─ src/api.ctx.md → src/api.ts 전용                       │
│  └─ src/utils/ctx.md → src/utils/ 폴더 전용                │
│                                                             │
└─────────────────────────────────────────────────────────────┘

우선순위: Local > Project > Global (더 구체적인 게 우선)
```

### 디렉토리 구조

#### Global (~/.ctx/) - 필수, Root, Git ❌
```
~/.ctx/
├── registry.yaml            # Global context + 전역 검색 인덱스 (유일한 파일!)
└── contexts/                # 전역 컨텍스트 (개인용)
    ├── coding-style.md      # 내 코딩 스타일
    ├── git-workflow.md      # Git 사용 패턴
    ├── typescript.md        # TS 선호 패턴
    └── tools/
        ├── docker.md
        └── aws.md
```

#### Project ({project}/.ctx/) - Git ✅ (팀 공유)
```
my-project/
├── .ctx/
│   ├── registry.yaml        # ⭐ Project/Local context (Git 관리!) - 유일한 파일!
│   └── contexts/            # 프로젝트 컨텍스트
│       ├── architecture.md
│       ├── api-patterns.md
│       └── business/
│           └── user-flow.md
├── docs/                    # ctx add docs/**/*.md 로 등록 가능
│   └── api-guide.md
├── src/
│   ├── api.ts
│   ├── api.ctx.md           # ← Local: api.ts 전용
│   └── utils/
│       ├── ctx.md           # ← Local: utils/ 폴더 전용
│       └── helpers.ts
└── ...
```

#### Local (*.ctx.md)
```
# 파일 컨텍스트
src/api.ts        ←→  src/api.ctx.md
src/auth/login.ts ←→  src/auth/login.ctx.md

# 폴더 컨텍스트
src/utils/        ←→  src/utils/ctx.md
src/components/   ←→  src/components/ctx.md
```

---

## 3. 핵심 기능 우선순위

### Load > Save > Sync

#### 1순위: Load (컨텍스트 주입)

**현재 문제**: 컨텍스트가 있어도 적시에 로드되지 않음

**해결책**: Claude Code Hook을 활용한 자동 주입

**Autoload 핵심 원칙**:
- **Path 기반, Programmatic** - target 필드로 매칭
- **현재 Project 기준** - Root index는 사용 안 함 (status --all 용)

**Autoload 우선순위**:
```
Read("src/api.ts") 호출
  │
  │  ※ target 없는 context는 Autoload 대상 아님
  │
  ├─ Tier 1: Exact Match (높은 우선순위)
  │   ├─ 1순위: Project registry - exact match
  │   │         target: "src/api.ts" → 정확히 일치
  │   └─ 2순위: Global registry - exact match
  │
  └─ Tier 2: Glob Match (낮은 우선순위)
      ├─ 3순위: Project registry - glob match
      │         target: "src/**/*.ts" → 패턴 매칭
      └─ 4순위: Global registry - glob match
```

**예시**:
```yaml
# Project registry
contexts:
  'src/api.ctx.md':
    target: 'src/api.ts'           # exact → 1순위
  '.ctx/contexts/ts-patterns.md':
    target: 'src/**/*.ts'          # glob  → 3순위

# Global registry
contexts:
  '~/.ctx/contexts/api-style.md':
    target: '**/api.ts'            # glob  → 4순위 (Global)
```

**동작 방식**:
```
1. cwd에서 위로 탐색 → .ctx/registry.yaml 찾기 (현재 Project)

2. Registry 읽기 + target 매칭
   ├─ Project registry의 contexts
   │   - target 있는 것만 추출
   │   - 읽은 파일과 exact/glob 매칭 체크
   │
   └─ Global registry의 contexts
       - target 있는 것만 추출
       - 읽은 파일과 exact/glob 매칭 체크

3. 우선순위대로 정렬하여 로드
   1순위: Project exact → 2순위: Global exact
   3순위: Project glob  → 4순위: Global glob
```

**트리거**: PostToolUse(Read) Hook → Hook Script → `ctx load --file`
- 파일 읽을 때마다 Hook 실행
- CLI는 매칭된 context **경로/메타데이터만 반환** (내용 X)
- Hook Script가 실제 파일 읽기 + 조합 담당
- `hookSpecificOutput.additionalContext`로 주입

#### 2순위: Save (컨텍스트 저장)

**현재 문제**: 언제/무엇을 저장해야 하는지 애매함

**해결책**: CLI + Skill 협업 방식 (→ Section 10 상세)

#### 3순위: Sync (검증 & 최신화)

**현재 문제**: sync가 파일 변경 감지에만 집중

**해결책**: 하이브리드 Registry 동기화 + 자동 Global Index 업데이트

```bash
# 기본 동작 (스마트 fallback)
ctx sync                    # Project 있으면 Project sync
                           # Project 없으면 자동으로 Global sync (경고 표시)

# 명시적 Global 동기화
ctx sync --global           # ~/.ctx/registry.yaml 동기화

# 전역 인덱스 강제 재빌드 (복구용)
ctx sync --rebuild-index    # 모든 프로젝트 스캔하여 index 재구축

# 헬스 체크
ctx check                   # 헬스 체크
ctx check --strict          # frontmatter 필수 검증
```

**Sync 기본 동작** (Project 없는 곳에서 실행 시):
```
$ ctx sync
⚠️ No project found in current directory
✓ Synced global registry (~/.ctx/registry.yaml)

💡 To create a project here: ctx init .
```

**Sync 플로우** (Project 내에서 실행):
```
ctx sync (프로젝트 내에서 실행)
  │
  ├─ 1. .ctx/registry.yaml의 등록된 context 파일들 확인
  │
  ├─ 2. 각 파일의 checksum 계산/비교
  │
  ├─ 3. .ctx/registry.yaml 갱신 (프로젝트 registry)
  │     - 변경된 파일 checksum 업데이트
  │     - 삭제된 파일 제거 (또는 경고)
  │     - 새 파일은 ctx add로 명시적 추가 필요
  │
  └─ 4. ⭐ Global index 자동 업데이트 (항상!)
        → ~/.ctx/registry.yaml의 index 섹션에
        → 현재 프로젝트 정보 갱신 (context_count, 요약 등)
```

**왜 Global Index를 자동 업데이트하는가?**
- 전역 조회(`ctx status --all`)의 신뢰성 보장
- "모든 context를 아는 본체"가 필요
- Lazy update는 조회 시 느려짐 → 미리 갱신이 효율적

**Context 등록 방식** (config 패턴 스캔 대신 명시적 등록):
```bash
# 개별 파일 등록
ctx add src/api.ctx.md

# glob 패턴으로 일괄 등록
ctx add docs/**/*.md
ctx add **/*.ctx.md

# 등록 해제
ctx remove test/*.ctx.md
```

---

## 4. Context 소속 결정 규칙

### 핵심 원칙

**모든 context 파일은 반드시 어딘가에 소속되며, 해당 registry에 등록된다.**

```
context 파일 위치에서 위로 탐색 → 첫 번째 .ctx/registry.yaml 발견
→ 그 Project에 소속 → Project registry에 등록
→ 못 찾으면 Global에 소속 → Global registry에 등록
```

### Context 소속 결정 플로우

```
┌─────────────────────────────────────────────────────────────┐
│               Context Ownership Detection                    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  파일: ~/projects/myapp/src/api.ctx.md                     │
│                                                             │
│  1. 현재 디렉토리부터 위로 탐색                             │
│                                                             │
│     ~/projects/myapp/src/                                  │
│        └─ .ctx/registry.yaml? ❌                            │
│                                                             │
│     ~/projects/myapp/                   ← Project 발견!    │
│        └─ .ctx/registry.yaml? ✅                            │
│                                                             │
│  결과: "projects/myapp" Project에 소속                     │
│        → ~/projects/myapp/.ctx/registry.yaml에 등록        │
│        → Git으로 팀과 공유됨 ✅                            │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  파일: ~/pg/hello.pdf.ctx.md  (project init 안 한 곳)      │
│                                                             │
│  1. 현재 디렉토리부터 위로 탐색                             │
│                                                             │
│     ~/pg/                                                   │
│        └─ .ctx/registry.yaml? ❌                            │
│                                                             │
│     ~/                                                      │
│        └─ .ctx/registry.yaml? ✅ (Global)                  │
│                                                             │
│  결과: Global에 소속                                       │
│        → ~/.ctx/registry.yaml에 등록                       │
│        → 개인용 (Git 미관리)                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 소속 결정 및 Registry 등록 알고리즘

```typescript
interface OwnerInfo {
  projectPath: string | null;  // null이면 Global
  registryPath: string;        // registry.yaml 경로
}

function findOwnerAndRegistry(contextFilePath: string): OwnerInfo {
  let dir = path.dirname(contextFilePath);
  const home = os.homedir();

  while (dir !== '/' && dir !== home) {
    const registryPath = path.join(dir, '.ctx', 'registry.yaml');
    if (fs.existsSync(registryPath)) {
      // Project 발견 → Project registry에 등록
      return {
        projectPath: dir,
        registryPath
      };
    }
    dir = path.dirname(dir);
  }

  // Global 소속 → Global registry에 등록
  return {
    projectPath: null,
    registryPath: path.join(home, '.ctx', 'registry.yaml')
  };
}
```

### Global = ~ (Home) Root

```
~/.ctx/registry.yaml 없음?
→ 에러: "Run 'ctx init' first to initialize global context"

경로 저장 규칙:
- Global registry: ~ 기준 상대경로
- Project registry: 프로젝트 루트 기준 상대경로
```

---

## 5. Config 없는 설계 (단순화)

### 왜 config.yaml이 필요 없는가?

기존 config.yaml의 역할과 대안:

| 기존 config 기능 | 새로운 방식 |
|------------------|-------------|
| `contexts.include` 패턴 | `ctx add docs/**/*.md` 명시적 등록 |
| `contexts.ignore` 패턴 | 등록 안 하면 됨 / `ctx remove` |
| `frontmatter.mode` | `ctx check --strict` CLI 옵션 |
| `auto.load` | hook 설치 여부로 결정 |
| `inherit` | 제거 (Load 시 항상 Global도 탐색) |

### 장점

1. **파일 1개로 단순화** - registry.yaml만 관리
2. **명시적 등록** - 의도치 않은 파일 등록 방지
3. **프로젝트 판단 단순화** - `.ctx/registry.yaml` 존재 여부로 판단
4. **설정 충돌 없음** - config 상속/오버라이드 복잡도 제거

### 프로젝트 마커

```
.ctx/registry.yaml 존재 → 이 디렉토리가 Project Root
~/.ctx/registry.yaml 존재 → Global CTX 초기화됨
```

### 스마트 Sync 동작

Project가 없는 디렉토리에서도 `ctx sync`가 자연스럽게 동작:

```
┌─────────────────────────────────────────────────────────────┐
│              ctx sync 스마트 Fallback                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  현재 위치에서 .ctx/registry.yaml 탐색                      │
│                                                             │
│  [발견됨] → Project sync 실행                               │
│            + Global index 자동 업데이트                     │
│                                                             │
│  [못 찾음] → 경고 표시 + Global sync 실행                   │
│            "⚠️ No project found, syncing global..."        │
│                                                             │
│  ✅ 어디서든 sync가 동작함 (에러 없음)                     │
│  ✅ Project 없어도 Global context 관리 가능                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Global Index 자동 업데이트

"모든 context를 아는 본체"가 필요하므로, 관련 명령어 실행 시 Global index가 자동 갱신됨:

| 명령어 | Global Index 업데이트 |
|--------|----------------------|
| `ctx sync` (Project) | ✅ 자동 |
| `ctx add` | ✅ 자동 |
| `ctx remove` | ✅ 자동 |
| `ctx create` | ✅ 자동 |
| `ctx sync --global` | ❌ (Global context만) |
| `ctx sync --rebuild-index` | ✅ 전체 재빌드 |

### Frontmatter 없는 문서 처리

기존 docs를 `ctx add`로 등록할 때 frontmatter가 없는 경우가 있다. 이 경우 자동 추출을 통해 `what` 필드를 채운다.

#### 추출 우선순위

```
1. frontmatter의 what 필드 (있으면 그대로 사용)
2. 첫 번째 # heading (마크다운 문서)
3. 파일명 humanize (api-guide.md → "Api Guide")
```

#### 예시

**Frontmatter 있는 경우:**
```markdown
---
what: "API 인증 가이드"
---
# Authentication
...
```
→ `what: "API 인증 가이드"` (frontmatter 우선)

**Frontmatter 없는 경우:**
```markdown
# API Authentication Guide

This document explains...
```
→ `what: "API Authentication Guide"` (heading에서 추출)

**Heading도 없는 경우:**
```markdown
This is a plain text document...
```
→ `what: "Api Guide"` (파일명 `api-guide.md`에서 추출)

#### CLI 출력

```bash
$ ctx add docs/**/*.md
✓ docs/api-guide.md (what: "API Authentication Guide")
✓ docs/setup.md (what: "Setup")  # 파일명 fallback
✓ docs/architecture.md (what: "시스템 아키텍처")  # frontmatter
Added 3 contexts

# strict 모드로 검증 시
$ ctx check --strict
✗ docs/setup.md: missing frontmatter (what was auto-extracted)
  Run: ctx add --with-frontmatter docs/setup.md
```

#### 장점

1. **기존 문서 즉시 등록** - frontmatter 없어도 바로 사용 가능
2. **점진적 개선** - 나중에 frontmatter 추가해도 됨
3. **예측 가능** - 추출 규칙이 명확함

---

## 6. 하이브리드 Registry

### 핵심 설계: 개인 + 팀 협업 모두 지원

```
┌─────────────────────────────────────────────────────────────┐
│                   하이브리드 Registry 구조                   │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ~/.ctx/registry.yaml           (Global Registry)           │
│  └─ Global context 관리                                    │
│  └─ 전역 검색용 인덱스 (캐시)                              │
│  └─ Git: ❌ (개인 머신 종속)                               │
│                                                             │
│  <project>/.ctx/registry.yaml   (Project Registry)          │
│  └─ Project context 관리                                   │
│  └─ Local context 관리                                     │
│  └─ Git: ✅ (팀 공유)                                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Global Registry (~/.ctx/registry.yaml)

```yaml
# ─────────────────────────────────────────────────────────
# Git: ❌ 미관리 (개인 머신 종속)
# 내용: Global context + 전역 검색용 인덱스
# ─────────────────────────────────────────────────────────
meta:
  version: "1.0.0"
  last_synced: "2025-12-30T10:00:00Z"

# ⭐ AI가 저장 위치 판단할 때 참조
settings:
  context_paths:
    - path: 'contexts/'
      purpose: "일반적인 개인 컨텍스트"
    - path: 'rules/'
      purpose: "코딩 규칙, 스타일 가이드, 린트 룰"
    - path: 'docs/'
      purpose: "문서화된 가이드, 레퍼런스"
    - path: 'tools/'
      purpose: "도구 사용법, CLI 치트시트"

contexts:
  # Global context만 관리
  '.ctx/contexts/coding-style.md':
    checksum: abc123
    what: "개인 코딩 스타일 가이드"

  '.ctx/contexts/tools/docker.md':
    checksum: def456
    what: "Docker 사용 패턴"

# ⭐ 전역 검색용 인덱스 (ctx sync 시 자동 업데이트!)
index:
  'projects/myapp':
    path: '/Users/me/projects/myapp'      # 절대 경로
    last_synced: "2025-12-31T12:00:00Z"   # 마지막 sync 시간
    context_count: 5
    # 요약 정보 (status --all 표시용)
    contexts:
      - path: 'src/api.ctx.md'
        what: "API 라우팅 로직"
      - path: '.ctx/contexts/architecture.md'
        what: "프로젝트 아키텍처"
      - path: 'src/auth/middleware.ctx.md'
        what: "인증 미들웨어"

  'projects/another':
    path: '/Users/me/projects/another'
    last_synced: "2025-12-30T15:00:00Z"
    context_count: 3
    contexts:
      - path: '.ctx/contexts/readme.md'
        what: "프로젝트 소개"
```

**Index 자동 업데이트 시점:**
- `ctx sync` 실행 시 (Project 내에서)
- `ctx add` / `ctx remove` 실행 시
- `ctx create` 실행 시

**Index의 역할:**
- 전역 검색 시 각 프로젝트 registry 파일 열지 않아도 됨
- `ctx status --all` 표시용
- "모든 context를 아는 본체" 역할

### Project Registry (<project>/.ctx/registry.yaml)

```yaml
# ─────────────────────────────────────────────────────────
# Git: ✅ 관리됨 (팀 공유)
# 내용: Project context + Local context
# ─────────────────────────────────────────────────────────
meta:
  version: "1.0.0"
  last_synced: "2025-12-30T10:00:00Z"

# ⭐ AI가 저장 위치 판단할 때 참조 (팀과 공유됨!)
settings:
  context_paths:
    - path: '.ctx/contexts/'
      purpose: "프로젝트 아키텍처, 설계 문서"
    - path: 'docs/'
      purpose: "API 문서, 사용자 가이드"
    - path: '.ctx/decisions/'
      purpose: "ADR, 기술 결정 기록"

contexts:
  # ─────────────────────────────────────────────────────────
  # target 없음 → Autoload 대상 아님
  # ─────────────────────────────────────────────────────────
  '.ctx/contexts/architecture.md':
    checksum: xyz789
    what: "프로젝트 아키텍처"

  'docs/api-guide.md':
    checksum: uvw012
    what: "API 가이드"

  # ─────────────────────────────────────────────────────────
  # target 있음 → Autoload 대상
  # exact path 또는 glob pattern 가능
  # ─────────────────────────────────────────────────────────

  # exact match - src/api.ts 읽을 때만 로드
  'src/api.ctx.md':
    target: 'src/api.ts'
    checksum: rst345
    what: "API 라우팅 로직"

  # glob pattern - src/**/*.ts 읽을 때 로드
  '.ctx/contexts/typescript-patterns.md':
    target: 'src/**/*.ts'
    checksum: ghi012
    what: "TypeScript 패턴 가이드"

  # glob pattern - 테스트 파일 읽을 때 로드
  '.ctx/contexts/test-guide.md':
    target: '**/*.test.ts'
    checksum: jkl345
    what: "테스트 작성 가이드"

  # folder target - src/utils/ 아래 파일 읽을 때 로드
  'src/utils/ctx.md':
    target: 'src/utils/'
    checksum: abc789
    what: "유틸리티 함수들"
```

### 저장 위치 결정 규칙

```
┌─────────────────────────────────────────────────────────────┐
│                  Save Location Decision                      │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  /ctx.save 실행 시 scope 선택                               │
│                                                             │
│  ┌─────────────┬──────────────────┬───────────────────────┐ │
│  │ Scope       │ 파일 저장 위치    │ Registry 위치         │ │
│  ├─────────────┼──────────────────┼───────────────────────┤ │
│  │ Global      │ ~/.ctx/contexts/ │ ~/.ctx/registry.yaml  │ │
│  │ Project     │ <proj>/.ctx/     │ <proj>/.ctx/registry.yaml  │ │
│  │ Local       │ <file>.ctx.md    │ <proj>/.ctx/registry.yaml  │ │
│  └─────────────┴──────────────────┴───────────────────────┘ │
│                                                             │
│  * Local/Project → 프로젝트 registry (Git 공유됨)          │
│  * Global → 글로벌 registry (개인용)                       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 조회 우선순위

```
┌─────────────────────────────────────────────────────────────┐
│                    Load Priority                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Autoload (target 매칭 시):                                 │
│                                                             │
│  1순위: Project registry - exact match                      │
│  2순위: Global registry - exact match                       │
│  3순위: Project registry - glob match                       │
│  4순위: Global registry - glob match                        │
│                                                             │
│  * target 필드가 있는 context만 Autoload 대상              │
│  * 더 구체적인 매칭(exact)이 우선                          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Registry 필드 규칙

| 위치 | 필드 | 의미 |
|------|------|------|
| Global registry | - | Global context |
| Project registry | `target` ❌ | Project context |
| Project registry | `target` ✅ | Local context (companion) |

### Settings 스키마

```yaml
settings:
  context_paths:              # AI가 저장 위치 판단할 때 참조
    - path: string            # 상대 경로 (registry 기준)
      purpose: string         # 이 경로의 용도 설명 (AI 판단용)
```

**용도**:
- AI Skill이 "어디에 저장할지" 판단할 때 `purpose`를 참고
- `ctx create` 시 경로 선택지로 제시
- 팀 프로젝트에서 context 저장 위치 표준화

**AI 판단 흐름 예시**:
```
사용자: "typescript rule 저장해줘"

AI 분석:
  1. scope 결정: Global (개인 코딩 스타일)
  2. ~/.ctx/registry.yaml의 settings.context_paths 확인
  3. "rule" 키워드 → purpose: "코딩 규칙..." 매칭
  4. → ~/.ctx/rules/typescript.md 추천
  5. ctx create --path rules/ typescript.md 실행
```

**기본값** (ctx init 시 생성):
```yaml
# Global (~/.ctx/registry.yaml)
settings:
  context_paths:
    - path: 'contexts/'
      purpose: "일반적인 개인 컨텍스트"

# Project (.ctx/registry.yaml)
settings:
  context_paths:
    - path: '.ctx/contexts/'
      purpose: "프로젝트 컨텍스트"
```

**Interactive 플로우** (ctx init 시):
```
$ ctx init .

📦 Initializing project context...

Where do you want to store contexts?
  Default: .ctx/contexts/

Add context paths (path:purpose, empty to finish):
  [1] .ctx/contexts/ : 프로젝트 컨텍스트 (default)
  [2] > docs/ : API 문서
  [3] > .ctx/decisions/ : ADR, 기술 결정
  [4] >

✓ Created .ctx/registry.yaml
  Paths:
    - .ctx/contexts/  → 프로젝트 컨텍스트
    - docs/           → API 문서
    - .ctx/decisions/ → ADR, 기술 결정
```

**Non-interactive** (AI용, --context-paths 옵션):
```bash
# AI가 프로젝트 초기화할 때
ctx init . --context-paths ".ctx/contexts/:프로젝트 컨텍스트,docs/:API 문서"
```

### Git 관리 정책

| 파일 | Git | 이유 |
|------|-----|------|
| `~/.ctx/registry.yaml` | ❌ | 개인 + 캐시 |
| `~/.ctx/contexts/**` | ❌ | 개인 컨텍스트 |
| `<proj>/.ctx/registry.yaml` | ✅ | **팀 컨텍스트 공유** |
| `<proj>/.ctx/contexts/**` | ✅ | 프로젝트 컨텍스트 |
| `<proj>/**/*.ctx.md` | ✅ | 로컬 컨텍스트 |

### 장점

1. **개인 + 팀 모두 지원** - 혼자 써도, 팀과 써도 OK
2. **Git으로 팀 공유** - 새 팀원 온보딩 시 clone만 하면 컨텍스트 바로 사용
3. **CI/CD 연동 가능** - 프로젝트 registry 기반으로 자동화 가능
4. **전역 검색 지원** - Global registry의 index 섹션 활용
5. **Orphan 처리** - 프로젝트 없는 context는 Global에 등록

---

## 7. 마이그레이션 계획

### 제거 대상
```
❌ src/templates/ai-commands/work/     # 전체 삭제
   ├── init.md
   ├── plan.md
   ├── extract.md
   ├── review.md
   ├── commit.md
   ├── submit.md
   └── done.md

❌ issue_store 관련 코드              # 전체 삭제
❌ worktree 관련 코드                 # 전체 삭제
❌ .ctx.current 관련 코드             # 전체 삭제
```

### As-Is → To-Be 매핑

| As-Is (현재) | To-Be (새 구조) |
|--------------|-----------------|
| `ctx/` 디렉토리 | `.ctx/contexts/` (Project) |
| `**/*.ctx.md` | 동일 (Local) |
| `ctx.config.yaml` (루트) | **제거** (config 없음) |
| 없음 | `~/.ctx/` (Global Root) |
| `local-context-registry.yml` | `.ctx/registry.yaml` (Project) |
| `global-context-registry.yml` | `~/.ctx/registry.yaml` (Global) |
| `work` 관련 전체 | 삭제 |

### 마이그레이션 스크립트
```bash
ctx migrate

# 수행 내용:
# 1. ~/.ctx/ 존재 확인 (없으면 ctx init 먼저 실행 안내)
# 2. 기존 ctx/ → .ctx/contexts/ 이동
# 3. 기존 ctx.config.yaml → 삭제 (더 이상 필요 없음)
# 4. 기존 local-context-registry.yml → .ctx/registry.yaml (Project)
# 5. 기존 global-context-registry.yml → ~/.ctx/registry.yaml (Global)
# 6. work 관련 파일 삭제 (백업 후)
# 7. .gitignore에서 .ctx/registry.yaml 제외 확인 (Git 관리되어야 함)
```

---

## 8. 커맨드 재설계

### CLI 커맨드

```bash
# ─────────────────────────────────────────
# 초기화
# ─────────────────────────────────────────
ctx init                    # ~/.ctx/ 생성 (Global Root, 최초 1회)
                           # + Claude Code plugin 설치 안내
                           # Interactive: context_paths 설정
ctx init .                  # .ctx/ 생성 (Project - registry.yaml만!)
                           # Interactive: context_paths 설정

# context_paths 직접 지정 (non-interactive, AI용)
ctx init --context-paths "contexts/:일반 컨텍스트,rules/:코딩 규칙"
ctx init . --context-paths ".ctx/contexts/:프로젝트 컨텍스트,docs/:API 문서"

# ─────────────────────────────────────────
# 컨텍스트 등록/해제 (NEW - config 대체)
# ─────────────────────────────────────────
ctx add <path>              # context 파일을 registry에 등록
ctx add docs/**/*.md        # glob 패턴으로 일괄 등록
ctx add --global <path>     # Global registry에 등록
                           # ⭐ Global index도 자동 업데이트!

ctx remove <path>           # registry에서 등록 해제
ctx remove docs/old/**      # glob 패턴으로 일괄 해제
                           # ⭐ Global index도 자동 업데이트!

# ─────────────────────────────────────────
# 컨텍스트 생성 (템플릿 기반)
# ─────────────────────────────────────────
ctx create <target>         # Local context 생성 → registry 등록 → Global index 갱신
ctx create --project <name> # Project context 생성 → registry 등록 → Global index 갱신
ctx create --global <name>  # Global context 생성 → Global registry 등록

# ─────────────────────────────────────────
# 컨텍스트 로드 (Hook용)
# ─────────────────────────────────────────
ctx load --file <path>      # target 매칭하여 context 경로/메타데이터 반환
                           # ex) ctx load --file src/api.ts
                           # → 매칭된 context 목록을 JSON으로 출력
                           # → 우선순위: Project exact > Global exact > Project glob > Global glob

ctx load --file <path> --json   # JSON 형식 출력 (기본값)
ctx load --file <path> --paths  # 경로만 출력 (줄바꿈 구분)

# ─────────────────────────────────────────
# 동기화 & 검증
# ─────────────────────────────────────────
ctx sync                    # 스마트 동작:
                           #   Project 있음 → Project sync + Global index 자동 갱신
                           #   Project 없음 → Global sync (경고 표시)
ctx sync --global           # Global registry만 동기화
ctx sync --rebuild-index    # Global index 강제 재빌드 (복구/초기화용)

ctx check                   # 헬스 체크 (파일 존재, checksum 일치)
ctx check --strict          # frontmatter 필수 검증 (기존 config의 역할)

# ─────────────────────────────────────────
# 상태 확인
# ─────────────────────────────────────────
ctx status                  # 현재 프로젝트 context 상태 + context_paths 표시
ctx status --global         # Global context만 (Global registry)
ctx status --all            # 전체 (index 기반)

# ─────────────────────────────────────────
# 마이그레이션
# ─────────────────────────────────────────
ctx migrate                 # 기존 구조 → 새 구조 변환
```

### AI Commands (Claude Code Skills)

```
/ctx.save [message]         # 통합 Save Skill (source 분기 처리)
                           # - 기본: 자연어 입력
                           # - --from session: Claude Code 세션에서 추출
                           # - --from slack: Slack 채널/스레드에서 추출 (MCP)
                           # - --from url: 웹페이지에서 추출
                           # - --from clipboard: 클립보드에서 추출
                           # → Skill이 scope 제안 → CLI create → 내용 작성

/ctx.load [keywords...]     # 컨텍스트 조회/로드 Skill
                           # - 자연어: "인증 관련 컨텍스트 찾아줘"
                           # - 키워드: /ctx.load auth jwt
                           # → ctx status로 목록 확인 → what 필드로 판단 → Read

/ctx.sync                   # ctx sync 래퍼
/ctx.status                 # ctx status 래퍼
```

**자동 로드 vs 수동 로드**:
- 자동 로드: Hook이 `ctx load --file` 호출하여 처리 (파일 읽을 때 자동)
- 수동 로드: `/ctx.load` Skill이 사용자 요청에 따라 context 검색/로드

---

## 9. Plugin 구조

### ctx repo 내부 서브패키지로 관리

```
ctx/
├── src/                      # CLI 코드
│   ├── commands/
│   │   ├── init.ts          # ctx init, ctx init .
│   │   ├── add.ts           # ctx add (NEW)
│   │   ├── remove.ts        # ctx remove (NEW)
│   │   ├── create.ts        # ctx create
│   │   ├── load.ts          # ctx load (NEW - 수동/자동 로드)
│   │   ├── sync.ts          # ctx sync
│   │   ├── check.ts         # ctx check
│   │   ├── status.ts        # ctx status
│   │   └── migrate.ts       # ctx migrate
│   └── lib/
│       ├── registry.ts      # registry 관리 (config 없음!)
│       └── loader.ts        # context 로딩
│
├── plugin/                   # Claude Code Plugin
│   ├── .claude-plugin/
│   │   └── plugin.json
│   ├── skills/
│   │   ├── save/
│   │   │   └── SKILL.md     # /ctx.save (통합: 자연어/session/slack/url/clipboard)
│   │   └── load/
│   │       └── SKILL.md     # /ctx.load (컨텍스트 조회/검색/로드)
│   ├── commands/
│   │   ├── sync.md          # /ctx.sync (CLI wrapper)
│   │   └── status.md        # /ctx.status (CLI wrapper)
│   ├── hooks/
│   │   ├── hooks.json                  # PostToolUse(Read) 자동 로드
│   │   └── auto-load-context.sh        # context 로드 스크립트
│   └── scripts/
│       └── auto-load-context.sh        # (hooks에서 참조)
│
└── package.json
```

### hooks.json 상세

Hook Script를 활용한 자동 로드:

```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Read",
        "hooks": [
          {
            "type": "command",
            "command": "plugin/hooks/auto-load-context.sh",
            "timeout": 5000
          }
        ]
      }
    ]
  }
}
```

**Hook Script** (`plugin/hooks/auto-load-context.sh`):
```bash
#!/bin/bash
# stdin에서 tool_input 읽기
input=$(cat)
file_path=$(echo "$input" | jq -r '.tool_input.file_path // empty')

if [ -z "$file_path" ]; then
  exit 0
fi

# CLI로 매칭되는 context 경로 조회
contexts=$(ctx load --file "$file_path" --json)

if [ -z "$contexts" ] || [ "$contexts" = "[]" ]; then
  exit 0
fi

# 각 context 파일 읽어서 조합
echo "$contexts" | jq -r '.[].path' | while read -r ctx_path; do
  echo "--- $ctx_path ---"
  cat "$ctx_path"
  echo ""
done
```

**동작 흐름**:
```
1. Claude Code가 Read tool로 파일 읽음
2. PostToolUse hook 트리거 → Hook Script 실행
3. Hook Script가 stdin에서 file_path 추출
4. ctx load --file 호출 → 매칭된 context 경로/메타 반환 (JSON)
5. Hook Script가 각 context 파일 읽어서 조합
6. stdout으로 출력 → additionalContext로 주입
```

**CLI 출력 예시** (`ctx load --file src/api.ts --json`):
```json
[
  {"path": "src/api.ctx.md", "what": "API 라우팅 로직"},
  {"path": ".ctx/contexts/ts-patterns.md", "what": "TypeScript 패턴 가이드"}
]
```

### Plugin 설치 방식

```bash
# ctx init 시 자동 설치 안내
ctx init

# 출력:
# ✓ Created ~/.ctx/
# ✓ Created ~/.ctx/registry.yaml
# ✓ Created ~/.ctx/contexts/
#
# 📦 Claude Code Plugin 설치:
#   claude plugins install ctx
# 또는 수동으로:
#   ln -s $(npm root -g)/ctx/plugin ~/.claude/plugins/ctx
```

---

## 10. Save 흐름 상세

### Skill vs Command 개념 정리

Claude Code에서 Skill과 Command는 다른 개념:

| 구분 | Skill | Command |
|------|-------|---------|
| **호출 방식** | 자연어 → Claude가 자동 선택 | `/command`로 명시적 호출 |
| **동작** | description과 요청 매칭 시 활성화 | 사용자가 입력할 때만 실행 |
| **구조** | SKILL.md + 리소스 파일들 | 단일 .md 파일 |
| **용도** | 복잡한 능력 부여 | 빠른 프롬프트 실행 |

**CTX 설계 원칙: Skill First**

```
┌─────────────────────────────────────────────────────────────┐
│                    Skill First 구조                          │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Skill (핵심)                                               │
│  └─ ctx-save: "이거 저장해줘" 같은 자연어에 자동 반응      │
│                                                             │
│  Command (편의용 Wrapper)                                   │
│  └─ /ctx.save → ctx-save Skill 명시적 호출                 │
│                                                             │
│  Hook (자동화)                                              │
│  └─ PostToolUse(Read) → 자동 context 로드                  │
│                                                             │
│  사용자 경험:                                               │
│  - 자연어로 말해도 됨 ("이 내용 컨텍스트로 저장해")        │
│  - 명시적으로 /ctx.save 해도 됨                            │
│  - 파일 읽으면 context 자동 로드됨                         │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Save 모드: Quick vs Deliberate

**핵심 문제**: Save의 마찰이 사용률을 결정함

```
저장의 마찰 > 저장의 즉각적 보상  →  사용 안 함
저장의 마찰 < 저장의 즉각적 보상  →  사용함
```

**해결책**: 두 가지 모드 제공

#### Quick Mode (Zero-Friction)

목적: 마찰 최소화, 습관화 불필요

```
┌─────────────────────────────────────────────────────────────┐
│                    Quick Save Flow                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  트리거:                                                    │
│  - "이거 저장해줘" (자연어)                                 │
│  - /ctx.save (명시적)                                       │
│  - 세션 종료 시 자동 제안 (Passive)                        │
│                                                             │
│  동작:                                                      │
│  1. Source: 현재 세션 (암묵적 기본값)                      │
│  2. 추출: AI가 자동 판단                                   │
│  3. 위치: AI가 추천 (1개)                                  │
│  4. 확인: "이거 맞죠?" → Y/n (1회)                         │
│                                                             │
│  사용자 입력: 최소 1회 (Y/n)                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### Deliberate Mode (정교한 제어)

목적: 외부 소스, 복잡한 구조화, 기존 컨텍스트 업데이트

```
┌─────────────────────────────────────────────────────────────┐
│                  Deliberate Save Flow                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  트리거:                                                    │
│  - "Slack에서 컨텍스트 추출해줘" (외부 소스 언급)          │
│  - "deliberate하게 저장하고 싶어" (명시적 요청)            │
│                                                             │
│  동작:                                                      │
│  1. Source: 사용자가 선택                                  │
│     - Claude session / Slack / URL / Clipboard / ...       │
│  2. 추출: 사용자가 리뷰/수정                               │
│  3. 위치: 후보 N개 제시 → 사용자 선택                      │
│  4. 생성/업데이트: 사용자 확인                             │
│  5. 메타데이터: 편집 가능                                  │
│                                                             │
│  사용자 입력: 단계별 확인                                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Save 워크플로우 파이프라인 (공통)

```
┌─────────────────────────────────────────────────────────────┐
│                    Save Pipeline                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. Source 탐색                                             │
│     └─ Claude session / Slack / URL / Clipboard / ...      │
│                                                             │
│  2. 유의미한 것 추출                                        │
│     └─ AI 분석 + 사용자 preference (optional)              │
│                                                             │
│  3. 저장 위치/방식 결정                                     │
│     └─ registry 기반으로 scope 판단                        │
│     └─ 새로 생성 vs 기존 컨텍스트 업데이트                 │
│                                                             │
│  4. 저장                                                    │
│     └─ 파일 생성/수정 + registry 등록                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Quick vs Deliberate 차이

| 단계 | Quick (Zero-Friction) | Deliberate |
|------|----------------------|------------|
| **1. Source** | 현재 세션 (암묵적) | 사용자가 명시적 선택 |
| **2. 추출** | AI가 자동 판단 | 사용자가 리뷰/수정 |
| **3. 위치** | AI 추천 (1개) → "맞죠?" | 후보 N개 → 사용자 선택 |
| **4. 생성/업데이트** | AI 판단 → 바로 실행 | 사용자 확인 필요 |
| **메타데이터** | 자동 생성 | 편집 가능 |
| **확인** | 최종 1회 (Y/n) | 단계별 확인 |

### 모드 선택 기준

> **TODO: DISCUSS** - 기본값 전략 및 구체적 UX 논의 필요

| 상황 | 추천 모드 | 이유 |
|------|----------|------|
| 세션 종료 시 자동 제안 | Quick | Passive Save |
| "이거 저장해줘" 한마디 | Quick | 마찰 최소화 |
| 외부 소스 (Slack/URL) | Deliberate | 소스 선택 필요 |
| 기존 컨텍스트 업데이트 | Deliberate | 충돌 확인 필요 |
| 복잡한 구조화 | Deliberate | 정교한 제어 필요 |

### Context Sources

다양한 소스에서 컨텍스트 추출 가능:

```
┌─────────────────────────────────────────────────────────────┐
│                    Context Sources                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  💬 Claude Code Session (기본값)                            │
│     - 현재 세션에서 유의미한 내용 추출                     │
│     - Quick mode의 암묵적 소스                             │
│                                                             │
│  💼 Slack (MCP 연동)                                        │
│     - 채널/스레드에서 추출                                 │
│                                                             │
│  📋 Clipboard                                               │
│     - 클립보드 내용에서 추출                               │
│                                                             │
│  🔗 URL                                                     │
│     - 웹페이지에서 추출                                    │
│                                                             │
│  📝 직접 입력                                               │
│     - 사용자가 자연어로 입력                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Load 플로우

```
┌─────────────────────────────────────────────────────────────┐
│                      Load Flow                               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  [자동 로드 - Hook] ⭐ 핵심 가치                           │
│  트리거: PostToolUse(Read) → Hook Script                   │
│  방식: Path 기반, Programmatic                             │
│  대상: target 필드가 있는 context만                        │
│                                                             │
│  Read("src/api.ts") 호출 시:                               │
│                                                             │
│  1. Hook Script가 stdin에서 file_path 추출                 │
│                                                             │
│  2. ctx load --file 호출                                   │
│     → CLI가 registry에서 target 매칭                       │
│     → 매칭된 context 경로/메타데이터 JSON 반환             │
│                                                             │
│  3. Hook Script가 각 context 파일 읽기                     │
│     → 내용 조합하여 stdout 출력                            │
│     → additionalContext로 주입                             │
│                                                             │
│  우선순위:                                                  │
│  1순위: Project exact → 2순위: Global exact                │
│  3순위: Project glob  → 4순위: Global glob                 │
│                                                             │
│  ※ target 없는 context → Autoload 대상 아님               │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  [수동 로드 - Skill] ⭐ 사용자 요청 기반                   │
│  트리거: "인증 관련 컨텍스트 찾아줘" (자연어)              │
│         /ctx.load auth jwt (명시적 호출)                   │
│  방식: ctx-load Skill 활성화 → 검색 → Read                │
│                                                             │
│  "인증 관련 컨텍스트 보여줘" 요청 시:                      │
│  1. ctx-load Skill 활성화                                  │
│  2. ctx status 실행 → context 목록 확인                   │
│  3. what 필드 보고 관련 context 판단                       │
│  4. Read tool로 직접 파일 읽기                             │
│  5. 사용자에게 context 내용 제공                          │
│                                                             │
│  ※ CLI에서 keyword search 구현 안 함                      │
│  ※ AI Skill이 registry 읽고 판단하는 방식                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Skills 구조

```
plugin/skills/
├── ctx-save/
│   └── SKILL.md              # Save Skill
│       description: "컨텍스트 저장, 추출, 기록 요청 시 활성화"
│       - Quick/Deliberate 모드 분기
│       - 다양한 source 지원
│       - scope 제안 + CLI create + 내용 작성
│
├── ctx-load/
│   └── SKILL.md              # Load Skill
│       description: "컨텍스트 조회, 검색, 로드 요청 시 활성화"
│       - ctx status로 목록 확인
│       - what 필드로 관련 context 판단
│       - Read tool로 파일 읽어서 제공
│
└── commands/                  # Skill의 명시적 진입점
    ├── ctx.save.md           # /ctx.save → ctx-save Skill 호출
    └── ctx.load.md           # /ctx.load → ctx-load Skill 호출
```

**자동 로드 vs 수동 로드**:
- 자동 로드: Hook Script가 `ctx load --file` 호출 (파일 읽을 때 자동)
- 수동 로드: `/ctx.load` Skill이 사용자 요청에 따라 context 검색/로드

### CLI vs Skill 역할

| 담당 | CLI | Skill |
|------|-----|-------|
| 파일 생성 | ctx create (템플릿) | - |
| 내용 작성 | - | Edit tool |
| Registry 동기화 | ctx sync | - |
| Scope 제안 | - | AI 분석 |
| 사용자 인터랙션 | - | 자연어 대화 |
| 검증 | ctx check | - |
| 모드 분기 (Quick/Deliberate) | - | AI 판단 |

---

## 11. 실행 계획

### Phase 1: 정리 및 구조 변경

1. **제거 작업**
   - `src/templates/ai-commands/work/` 전체 삭제
   - issue_store 관련 코드 삭제
   - worktree 관련 코드 삭제
   - `.ctx.current` 관련 코드 삭제
   - **config.yaml 관련 코드 삭제**

2. **디렉토리 구조 변경**
   - `ctx/` → `.ctx/` 마이그레이션 로직
   - `ctx.config.yaml` → 삭제 (config 없음)

3. **커맨드 정리**
   - `ctx init` 수정 (Global Root 초기화, registry만 생성)
   - `ctx init .` 추가 (Project 초기화, registry만 생성)
   - `ctx add` / `ctx remove` 추가 (config 패턴 대체)
   - work 관련 커맨드 제거

### Phase 2: Registry 단일화 구현

1. **Global Root 구현**
   - `~/.ctx/` 디렉토리 지원
   - **registry.yaml만 관리 (config 없음!)**

2. **Context 소속 로직**
   - registry.yaml 존재 여부로 project 판단
   - 위로 탐색하여 project 결정

3. **Registry 통합**
   - 기존 분산 registry → 하이브리드 registry 마이그레이션
   - ctx add/remove로 명시적 등록

### Phase 3: Plugin 구조화

1. **Plugin 디렉토리 생성**
   - `plugin/` 디렉토리 구조
   - plugin.json 작성

2. **Skills 구현**
   - `/ctx.save` skill

3. **Hooks 구현**
   - PostToolUse(Read) 자동 로드 hook
   - Hook Script (`auto-load-context.sh`)

4. **설치 연동**
   - `ctx init` 시 plugin 설치 안내

### Phase 4: Load 자동화

1. **PostToolUse hook 구현**
   - Read tool 호출 시 companion context 자동 주입
   - `.ctx.md` 파일이 있으면 함께 표시

2. **Status 개선**
   - `ctx status` 출력 개선
   - what 필드로 context 설명 표시

3. **SessionStart hook** (선택적)
   - 프로젝트 진입 시 핵심 컨텍스트 요약

---

## 12. 결론

### 핵심 변경사항

| 항목 | Before | After |
|------|--------|-------|
| Work workflow | 7단계 복잡한 흐름 | **완전 제거** |
| Issue store | local/github/linear | **제거** |
| Worktree | 지원 | **제거** |
| Config | `ctx.config.yaml` | **제거 → settings로 최소화** |
| Registry | 분산 (project마다) | **하이브리드** (Global + Project) |
| Global | 없음 | `~/.ctx/` (필수, Root, Git ❌) |
| Project registry | 없음 | `.ctx/registry.yaml` (Git ✅) |
| Project init | 암묵적 | `ctx init .` (registry만!) |
| Context 등록 | config 패턴 스캔 | **`ctx add` 명시적 등록** |
| 저장 위치 판단 | 없음 | **`settings.context_paths`** (AI 참조) |
| Save 실행 | AI가 직접 파일 작성 | CLI 템플릿 + Skill 내용 |
| 팀 협업 | 제한적 | **Git으로 registry 공유** |
| Sync 동작 | Project 없으면 에러 | **스마트 Fallback** (Global로) |
| Global Index | 수동 (`--index`) | **자동 업데이트** (sync/add/create 시) |

### 핵심 원칙

1. **Load가 핵심이다**
   - 컨텍스트가 자동으로 눈앞에 나타나야 함
   - 사용자가 "로드해야지" 생각 안 해도 되게

2. **저장은 자연스럽게**
   - "이거 저장해둘래" 한마디면 끝
   - Skill이 scope 제안, CLI가 템플릿 생성

3. **단순함이 최우선**
   - Work, issue store, worktree 모두 제거
   - **config.yaml 제거** - registry.yaml 하나로 충분
   - 핵심 기능(load, save, sync)에 집중

4. **명시적 등록** (NEW)
   - config 패턴 스캔 대신 `ctx add`로 명시적 등록
   - 의도치 않은 파일 등록 방지
   - 등록된 것만 관리 (예측 가능)

5. **모든 context는 소속이 있다**
   - project init 안 해도 context 생성 가능
   - 위로 탐색 → project 없으면 Global 소속
   - Orphan 없음

6. **개인 + 팀 모두 지원**
   - Global registry: 개인용 (Git ❌)
   - Project registry: 팀 공유 (Git ✅)
   - 혼자 써도 가치, 팀과 써도 가치

7. **스마트 Fallback** (NEW)
   - Project 없는 곳에서도 `ctx sync` 동작
   - 에러 대신 경고 + Global sync
   - 어디서든 자연스럽게 사용 가능

8. **Global Index = 본체** (NEW)
   - "모든 context를 아는 중앙 인덱스"
   - sync/add/create 시 자동 업데이트
   - 전역 검색의 신뢰성 보장
   - Lazy update 대신 선제적 갱신

### 성공 지표

- 컨텍스트 로드를 "의식하지 않아도" 자동으로 됨
- 저장할 때 "어디에?" 고민 안 해도 됨
- CLI 설치 후 5분 내 사용 가능
- 혼자 써도 가치를 느낌
- `ctx status` 한 번에 모든 context 파악
- **전역 검색이 항상 최신 상태** - sync 후 바로 반영 (NEW)
- **어디서든 sync 가능** - Project 없어도 동작 (NEW)
- **새 팀원 온보딩: clone만 하면 컨텍스트 바로 사용**
- **CI/CD에서 컨텍스트 활용 가능**
